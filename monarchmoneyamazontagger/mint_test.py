...
class HelpMethods(unittest.TestCase):
    def test_truncate_title(self):
        self.assertEqual(
            mint.truncate_title("Some great title [", 20), "Some great title"
        )
        self.assertEqual(mint.truncate_title(" Some great title abc", 5), "Some")
    ...
    def test_convertCamel_to_underscores(self):
        ...
        self.assertEqual(mint.convertCamel_to_underscores("oneword"), "oneword")
        self.assertEqual(mint.convertCamel_to_underscores("CapCase?"), "cap_case?")

    def test_parse_mint_date(self):
        self.assertEqual(mint.parse_mint_date("2020-01-10"), date(2020, 1, 10))
        self.assertEqual(mint.parse_mint_date("2022-11-30"), date(2022, 11, 30))
        self.assertEqual(mint.parse_mint_date("2019-10-08"), date(2019, 10, 8))
...
class TransactionClass(unittest.TestCase):
    ...
    def test_get_compare_tuple(self):
        trans = transaction(description="Simple Title", amount=-1.00)
        self.assertEqual(
            trans.get_compare_tuple(),
            ("Simple Title", "-$1.00", "Great note here", "Personal Care"),
        )

        trans2 = transaction(description="Simple Refund", amount=2.01)
        self.assertEqual(
    ...
    def test_sum_amounts(self):
        ...
        credit = transaction(amount=20.20)
        self.assertEqual(Transaction.sum_amounts([trans1, credit, trans2]), 9860000)

    def test_unsplit(self):
        self.assertEqual(Transaction.unsplit([]), [])

        not_child1 = transaction(amount=-1.00)
        self.assertEqual(Transaction.unsplit([not_child1]), [not_child1])

        not_child2 = transaction(amount=-2.00)
        self.assertEqual(
            Transaction.unsplit([not_child1, not_child2]), [not_child1, not_child2]
        )

        child1_to_1 = transaction(amount=-3.00, parent_id=1)
        child2_to_1 = transaction(amount=-4.00, parent_id=1)
        child3_to_1 = transaction(amount=8.00, parent_id=1)
        child1_to_99 = transaction(amount=-5.00, parent_id=99)
    ...
    def test_old_and_new_are_identical(self):
        ...
        trans2 = transaction(amount=-5.00, description="ABC", category="Shipping")

        self.assertTrue(Transaction.old_and_new_are_identical(trans1, [trans1]))
        self.assertFalse(Transaction.old_and_new_are_identical(trans1, [trans2]))
        self.assertTrue(Transaction.old_and_new_are_identical(trans1, [trans2], True))
        ...
        self.assertTrue(Transaction.old_and_new_are_identical(trans1, new_trans))
    ...
    def test_summarize_new_trans(self):
        original_trans = transaction(
            amount=-40.00, description="Amazon", notes="Test note"
        )

        item1 = transaction(amount=-15.00, description="Item 1")
        item2 = transaction(amount=-25.00, description="Item 2")
        shipping = transaction(amount=-5.00, description="Shipping")
        free_shipping = transaction(amount=-5.00, description="Promotion(s)")

        actual_summary = mint.summarize_new_trans(
            original_trans, [item1, item2, shipping, free_shipping], "Amazon.com: "
        )[0]

        self.assertEqual(actual_summary.amount, original_trans.amount)
        self.assertEqual(actual_summary.category.name, category.DEFAULT_MINT_CATEGORY)
    ...
    def test_summarize_new_trans_one_item_keeps_category(self):
        ...
        actual_summary = mint.summarize_new_trans(
            original_trans, [item1, shipping, free_shipping], "Amazon.com: "
        )[0]

        self.assertEqual(actual_summary.amount, original_trans.amount)
        self.assertEqual(actual_summary.category.name, "Office Supplies")
        self.assertEqual(actual_summary.description, "Amazon.com: Giant paper shredder")
