...
# This tool takes an Amazon Data Export and annotates Monarch Money
...
from PyQt6.QtGui import QDesktopServices, QKeySequence, QShortcut  # type: ignore
...
from monarchmoneyamazontagger.args import (
    define_gui_args,
    get_name_to_help_dict,
    TAGGER_BASE_PATH,
)
from monarchmoneyamazontagger.qt import (
    MMUpdatesTableModel,
    AmazonUnmatchedTableDialog,
    AmazonStatsDialog,
    TaggerStatsDialog,
)
...
class TaggerGui:
    ...
    def create_gui(self):
        ...
        app.setStyle("Fusion")
        version_string = f"Monarch Money Amazon Tagger v{VERSION}"
        ...
        #             "experience. "
        #             "https://github.com/jprouty/monarchmoney-amazon-tagger"
        #         )
        #         logger.warning(
        ...
        mm_group = QGroupBox("Monarch Money Login && Options")
        mm_group.setMinimumWidth(350)
        mm_layout = QFormLayout()

        mm_layout.addRow(
            self.create_line_label("Email:", "mm_email"),
            self.create_line_edit("mm_email", tool_tip=NEVER_SAVE_MSG),
        )
        mm_layout.addRow(
            self.create_line_label("Password:", "mm_password"),
            self.create_line_edit(
                "mm_password", tool_tip=NEVER_SAVE_MSG, password=True
            ),
        )
        mm_layout.addRow(
            self.create_line_label("Sync first?", "mm_wait_for_sync"),
            self.create_checkbox("mm_wait_for_sync"),
        )

        mm_layout.addRow(
            self.create_line_label("Description Filter", "mm_input_description_filter"),
            self.create_line_edit("mm_input_description_filter"),
        )
        mm_layout.addRow(
            self.create_line_label(
                "Include user description", "mm_input_include_user_description"
            ),
            self.create_checkbox("mm_input_include_user_description"),
        )
        mm_layout.addRow(
            self.create_line_label(
                "Include inferred description",
                "mm_input_include_inferred_description",
            ),
            self.create_checkbox("mm_input_include_inferred_description"),
        )
        mm_layout.addRow(
            self.create_line_label(
                "Input Categories Filter", "mm_input_categories_filter"
            ),
            self.create_line_edit("mm_input_categories_filter"),
        )
        mm_group.setLayout(mm_layout)
        h_layout.addWidget(mm_group)
        ...
        tagger_right.addRow(
            self.create_line_label("Do not tag categories", "no_tag_categories"),
    ...
    def create_amazon_import_layout(self):
        ...
        order_history_link.setText(
            ...
            Wait for the export to complete<br>
            Download the file to your computer<br>
        ...
        amazon_import_layout.addRow(
            ...
            self.create_file_edit("amazon_export", "Select Amazon Data Export"),
        )
    ...
    def on_start_button_clicked(self):
        ...
        if not getattr(self.args, "mm_email") or not getattr(self.args, "mm_password"):
            error_dialog = QErrorMessage(self.window)
            error_dialog.showMessage("Please provide an email and password.")
            self.on_tagger_dialog_closed()
            return

        self.tagger = TaggerDialog(
            args=args, parent=self.window, log_filename=self.log_filename
        )
        self.tagger.show()
    ...
    def create_date_edit(
        self, name, popup_title, max_date=datetime.date.today(), tool_tip=None
    ):
    ...
    def create_file_edit(
        self, name, popup_title, filter="Zip files (*.zip)", tool_tip=None
    ):
        label = "Select a file"
        ...
        def on_button():
            dlg = QFileDialog()
            selection = dlg.getOpenFileNames(self.window, popup_title, filter=filter)
            if selection[0]:
                ...
                label = " AND ".join(
                    [os.path.split(file.name)[1] for file in new_files]
                )
...
class TaggerDialog(QDialog):
    def __init__(self, args, log_filename, **kwargs):
        ...
        self.thread.started.connect(partial(self.worker.create_updates, args, self))
        self.thread.start()
    ...
    def on_report_issue(self):
        ...
        webbrowser.open(
            "https://github.com/jprouty/monarchmoney-amazon-tagger/issues/"
    ...
    def on_double_click(self, index):
        ...
        order_id = self.updates_table_model.data(
            order_id_cell, Qt.ItemDataRole.DisplayRole
        )
        self.open_amazon_order_id(order_id)
    ...
    def on_review_ready(self, results):
        ...
        self.label.setText("Select below which updates to send to Monarch Money.")

        self.updates_table_model = MMUpdatesTableModel(results.updates)
        ...
        self.updates_table.setSelectionMode(
        ...
        amazon_stats_button.clicked.connect(
            partial(self.on_open_amazon_stats, results.items, results.charges, [])
        )
        ...
        self.confirm_button = QPushButton("Send to Monarch Money")
    ...
    def on_updates_sent(self, num_sent):
        self.label.setText(f"All done! {num_sent} newly tagged transactions")
        self.cancel_button.setText("Close")
    ...
    def on_mfa(self):
        mfa_code, ok = QInputDialog().getText(
            self, "Please enter your MFA/OTP Code.", "Code:"
        )
...
class TaggerWorker(QObject):
    ...
    def do_create_updates(self, args, parent):
        # Factory that handles indeterminate, determinate, and counter style.
        def progress_factory(msg, max=0):
            return QtProgress(msg, max, self.on_progress.emit)

        self.mmc = MonarchMoneyClient(args)
    ...
    def do_send_updates(self, updates, args):
        num_updates = self.mmc.send_updates(
            updates,
            progress=QtProgress(
                "Sending updates to Monarch Money", len(updates), self.on_progress.emit
            ),
...
def main():
    ...
    sys.exit(TaggerGui(args, get_name_to_help_dict(parser), log_filename).create_gui())
